...
async function callClaudeForFG(d, apiKey, history=[]) {
  ...
  const historyText = history.length > 0
    ? `\n[Firebase 누적 히스토리 — 실제 저장된 ${history.length}일 데이터 (최신순), 이 데이터 전체를 추세 분석에 활용하세요]\n` +
...
[작성 형식]
맨 앞에 두 줄 요약 후 --- 이후 전체 리포트 (총 1200토큰 이내로 반드시 완성):
📊 [첫째 줄 요약]
📈 [둘째 줄 요약]
---
**😱 Fear & Greed 지수 분석**
- 현재 점수/등급 의미 (2~3문장)
- 누적 히스토리 기반 추세: 날짜 나열 없이 "N거래일간 X~Y점 박스권" 형태로 한 줄 요약
- 전일/1주/1개월/1년 방향성: 나열 없이 "단기 상승·중기 하락·장기 안정" 형태로 한 줄 요약
- 역사적 패턴: 긍정/부정 시나리오 각 2문장씩 (확률 포함)

**📊 종합 의견** (3~4문장)
- 현재 심리 구간 평가 + 앞으로 방향성 + 투자자 주의사항
