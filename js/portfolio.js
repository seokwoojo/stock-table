// ─────────────── PORTFOLIO ───────────────
// Fixed accounts always displayed (not deletable)
...
function ensureFixedPortfolios(){
  ...
  // Keep fixed accounts in order at top
...
function addStock(portfolioId){
  ...
  p.stocks.push({
    id:uid(), name:'', 
    baseQty:0, baseAvgPrice:0,  // 기준 포지션 (직접 입력)
    curPrice:0, dividend:0, dividendCycle:'연', dividendDate:'',
    accumulatedDividend:0, monthlyBuy:0,
    trades:[]  // { id, date, type:'매수'|'매도', qty, price }
  });
  renderPortfolios();
}
...
function updateStock(pid, sid, field, value){
  ...
  if(field === 'dividendCycle' || field === 'dividendDate'){
    // 배당 주기/날짜 변경은 UI 구조가 바뀌므로 전체 재렌더
    renderPortfolios();
    renderSavings();
    recalcAll();
  } else if(numFields.includes(field)){
    // 숫자 필드: 계산 결과 셀만 DOM 직접 업데이트 (재렌더 없음 → 포커스 유지)
...
function updateStockCells(p, s){
  ...
  const expDiv = val*(periodDiv/100);

  const row = document.querySelector(`tr.stock-row[data-sid="${s.id}"]`);
  if(!row) return;
  const cells = row.querySelectorAll('td');
  // 0종목명 1수량 2평단가 3현재가 4평가금액 5미실현 6주가수익률 7실현수익 8배당률 9배당주기 10배당일 11예상배당 12실배당 13누적배당 14총수익률 15월매수금 16버튼
  ...
  cells[5].textContent = (unreal>=0?'+':'')+('₩'+Math.abs(unreal).toLocaleString('ko-KR'));
  ...
  cells[11].textContent = expDiv>0?fmtKRW(Math.round(expDiv)):'—';
  cells[11].className = `mono ${expDiv>0?'pos':''}`;
  cells[14].innerHTML = `<span class="badge ${isTotPos?'badge-green':'badge-red'}">${isTotPos?'+':''}${tr2}%</span>`;
}
...
function updatePortfolioHeader(p){
  ...
  if(stats[0]) stats[0].textContent = totVal ? '₩'+totVal.toLocaleString('ko-KR') : '—';
  if(stats[1]){
    stats[1].textContent = (totUnreal>=0?'+':'')+fmtKRW(totUnreal)+' ('+(totUnreal>=0?'+':'')+pnlRate+'%)';
    stats[1].className = `val ${totUnreal>=0?'pos':'neg'}`;
  }
  if(stats[2]){
    stats[2].textContent = totReal!==0?(totReal>=0?'+':'')+fmtKRW(Math.round(totReal)):'—';
    stats[2].className = `val ${totReal>=0?'pos':'neg'}`;
  }
  if(stats[3]){
    stats[3].textContent = (totTotal>=0?'+':'')+fmtKRW(Math.round(totTotal))+' ('+(totTotal>=0?'+':'')+totRate+'%)';
    stats[3].className = `val ${totTotal>=0?'pos':'neg'}`;
  }
}
  ...
  sec.innerHTML = state.portfolios.map(p => {
    ...
    const nameEl = isFixed
      ? `<span class="account-badge" style="background:var(--accent-dim);color:var(--accent);border:1px solid rgba(0,230,118,0.3);">${p.type}</span>
         <input class="portfolio-name-input"
           style="background:transparent;border:none;border-bottom:1px dashed rgba(0,230,118,0.3);color:var(--text);font-weight:700;font-size:14px;padding:2px 6px;min-width:120px;"
      ...
      : `<span class="account-badge">${p.type}</span>
         <input class="portfolio-name-input"
           style="background:transparent;border:none;border-bottom:1px dashed var(--border);color:var(--text);font-weight:700;font-size:14px;padding:2px 6px;min-width:120px;"
           value="${p.accountName}" onclick="event.stopPropagation()"
           data-cb="${`updatePortfolioName(${p.id},v)`.replace(/"/g,'&quot;')}"
           oninput="nInputText(this)" onblur="nBlurText(this)"
           onfocus="this.style.borderColor='var(--accent)'">`;
    ...
    const stockRows = p.stocks.length === 0
      ? `<tr class="empty-row"><td colspan="17">종목이 없습니다 — 아래 버튼으로 추가하세요</td></tr>`
      : p.stocks.map(s => {
          const pos      = calcPosition(s);
          const qty      = pos.qty;
          const avgPrice = pos.avgPrice;
          ...
          const tradeLog = (s.trades||[]).map(t => {
            const avgAtTrade = (() => {
              let q = s.baseQty||0, cb = q*(s.baseAvgPrice||0);
              for(const tt of (s.trades||[])){
                if(tt.id===t.id) break;
                if(tt.type==='매수'){ q+=tt.qty; cb+=tt.qty*tt.price; }
                else { const ac=q>0?cb/q:0; q-=tt.qty; cb-=ac*tt.qty; }
              }
              return q>0?cb/q:0;
            })();
            const tpnl = t.type==='매도' ? (t.price-avgAtTrade)*t.qty : null;
            return `<tr>
              <td class="${t.type==='매수'?'trade-buy':'trade-sell'}">${t.type}</td>
              <td>${t.date||'—'}</td>
              <td class="mono">${t.qty.toLocaleString()}주</td>
              <td class="mono">${fmtKRW(t.price)}</td>
              <td class="mono">${fmtKRW(t.qty*t.price)}</td>
              <td class="mono ${tpnl!==null?(tpnl>=0?'pos':'neg'):''}">${tpnl!==null?(tpnl>=0?'+':'')+fmtKRW(Math.round(tpnl)):'—'}</td>
              <td><button class="btn btn-danger" style="padding:2px 6px;font-size:11px;" onclick="removeTrade(${p.id},${s.id},${t.id})">✕</button></td>
            </tr>`;
          }).join('');
          ...
          <tr class="stock-row" data-sid="${s.id}">
            ...
            <td class="mono ${expDiv>0?'pos':''}">${expDiv>0?fmtKRW(Math.round(expDiv)):'—'}</td>
            <td>
            ...
            <td>
              <div style="display:flex;gap:4px;">
                <button class="trade-btn trade-btn-buy" style="padding:4px 8px;" onclick="toggleTradePanel(${s.id})">거래</button>
                <button class="btn btn-danger" onclick="removeStock(${p.id},${s.id})">✕</button>
              </div>
            </td>
          </tr>
          <tr><td colspan="17" style="padding:0;">
            <div class="trade-panel" id="tp-${s.id}">
              <div class="trade-panel-inner">
                <div style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--text3);margin-bottom:12px;">📋 ${s.name||'종목'} 거래 내역</div>
                <div style="background:var(--surface);border:1px solid var(--border);border-radius:3px;padding:10px 14px;margin-bottom:12px;">
                  <div style="font-size:10px;color:var(--muted);letter-spacing:1px;text-transform:uppercase;margin-bottom:8px;">기준 포지션 (이전 거래 합산 입력)</div>
                  <div style="display:flex;gap:16px;align-items:center;flex-wrap:wrap;">
                    <div style="display:flex;align-items:center;gap:6px;">
                      <span style="font-size:11px;color:var(--muted);">수량</span>
                      ${mkNum(s.baseQty, `updateStock(${p.id},${s.id},'baseQty',v)`, 'max-width:80px;', '0')}
                      <span style="font-size:11px;color:var(--muted);">주</span>
                    </div>
                    <div style="display:flex;align-items:center;gap:6px;">
                      <span style="font-size:11px;color:var(--muted);">평단가</span>
                      ${mkNum(s.baseAvgPrice, `updateStock(${p.id},${s.id},'baseAvgPrice',v)`, 'max-width:100px;', '0')}
                      <span style="font-size:11px;color:var(--muted);">원</span>
                    </div>
                    <div style="font-family:var(--mono);font-size:11px;color:var(--muted);">
                      → 현재: <span style="color:var(--text2);font-weight:600;">${qty}주 @ ${avgPrice?fmtKRW(avgPrice):'—'}</span>
                    </div>
                  </div>
                </div>
                <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:10px;">
                  <button class="trade-btn trade-btn-buy" onclick="addTrade(${p.id},${s.id},'매수')">＋ 매수</button>
                  <button class="trade-btn trade-btn-sell" onclick="addTrade(${p.id},${s.id},'매도')">－ 매도</button>
                  ${mkDate('', `updateMaturity(-1,'_',v)`, `id="tr-date-${s.id}" style="max-width:120px;"`)}
                  <div style="display:flex;align-items:center;gap:4px;">
                    <input type="text" inputmode="numeric" id="tr-qty-${s.id}" placeholder="수량" style="max-width:70px;font-family:var(--mono);font-size:12px;" onfocus="this.select()">
                    <span style="font-size:11px;color:var(--muted);">주</span>
                  </div>
                  <div style="display:flex;align-items:center;gap:4px;">
                    <input type="text" inputmode="numeric" id="tr-price-${s.id}" placeholder="가격" style="max-width:90px;font-family:var(--mono);font-size:12px;" onfocus="this.select()">
                    <span style="font-size:11px;color:var(--muted);">원</span>
                  </div>
                </div>
                ${(s.trades||[]).length>0?`
                <table class="trade-log">
                  <thead><tr><th>구분</th><th>날짜</th><th>수량</th><th>가격</th><th>금액</th><th>실현손익</th><th></th></tr></thead>
                  <tbody>${tradeLog}</tbody>
                  <tfoot>
                    <tr style="background:var(--surface2);">
                      <td colspan="5" style="font-family:var(--mono);font-size:11px;color:var(--muted);padding:6px 10px;">누적 실현손익</td>
                      <td class="mono ${isRealPos?'pos':'neg'}" style="padding:6px 10px;font-weight:700;">${isRealPos?'+':''}${fmtKRW(Math.round(realPnl))}</td>
                      <td></td>
                    </tr>
                  </tfoot>
                </table>`:`<div style="font-size:12px;color:var(--muted);font-family:var(--mono);padding:8px 0;">아직 거래 내역이 없습니다</div>`}
              </div>
            </div>
          </td></tr>`;
        }).join('');
    ...
    <div class="account-panel" id="ph-${p.id}" style="${isFixed?'border-color:rgba(0,230,118,0.15);':''}">
      <div class="account-header" ${headerClickAttr} style="${isFixed?'cursor:default;':''}">
        ...
        <div class="account-header-right">
          ...
          <div class="account-stat"><div class="label">종목 수</div><div class="val">${p.stocks.length}종목</div></div>
          ${typeSelect}${deleteBtn}${chevEl}
        </div>
      </div>
      <div class="account-body" id="pb-${p.id}">
        <div style="overflow-x:auto;-webkit-overflow-scrolling:touch;">
        <table style="min-width:1200px;" class="portfolio-table">
          <colgroup>
            ...
            <col style="width:85px;"><col style="width:90px;"><col style="width:110px;">
            <col style="width:95px;"><col style="width:120px;"><col style="width:95px;">
            <col style="width:75px;"><col style="width:95px;"><col style="width:80px;">
          </colgroup>
          <thead>
            <tr>
              ...
              <th>예상 배당금</th><th>실제 배당 지급액</th><th>누적 배당금</th>
...
function updatePortfolioType(id, val){
  ...
  // 연결된 저축 카드 타입/이름도 동기화
  if(!p.fixed){
    const s = state.savings.find(x=>x.id===id);
    if(s){
      s.type = val;
      s.name = p.accountName;
    }
  }
  renderPortfolios();
  renderSavings();
...
// ─────────────── 거래 계산 ───────────────
// 기준 포지션 + trades → 현재 수량, 평단가, 실현수익 계산 (FIFO)
function calcPosition(s){
  const trades = s.trades || [];
  // 기준 포지션을 첫 매수로 취급
  let qty = s.baseQty || 0;
  let costBasis = qty * (s.baseAvgPrice || 0); // 총 원가
  let realizedPnl = 0;
...
function addTrade(pid, sid, type){
  const p = state.portfolios.find(x=>x.id===pid);
  if(!p) return;
  const s = p.stocks.find(x=>x.id===sid);
  if(!s) return;

  const dateEl = document.getElementById(`tr-date-${sid}`);
  const qtyEl  = document.getElementById(`tr-qty-${sid}`);
  const priceEl= document.getElementById(`tr-price-${sid}`);
  ...
  const pos = calcPosition(s);
  if(type === '매도' && qty > pos.qty){
    showToast(`⚠️ 보유 수량(${pos.qty}주)보다 많이 매도할 수 없습니다`); return;
  }

  const stockName = s.name || '종목';
  const label = type === '매수' ? '매수' : '매도';
  if(!confirm(`${stockName} ${label} ${qty}주 @ ${fmtKRW(price)}\n총 ${fmtKRW(qty*price)}\n거래를 추가할까요?`)) return;
  ...
  recalcAll();
  scheduleSave();
  showToast(`✅ ${label} 거래 추가됨`);
}

function removeTrade(pid, sid, tid){
  const p = state.portfolios.find(x=>x.id===pid);
  if(!p) return;
  const s = p.stocks.find(x=>x.id===sid);
  if(!s) return;
...
function toggleTradePanel(sid){
  const panel = document.getElementById(`tp-${sid}`);
